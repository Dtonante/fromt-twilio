...
//     // elementos agregados en el HTML
...
//                 url: 'https://back-twilio-54644131242.us-central1.run.app/voice',
//                 params: {
//                     To: params.To
//                 } 
//             });
...
//                 'https://back-twilio-54644131242.us-central1.run.app/voice-token'
//             );
//             log('Token obtenido.');
//             accessToken = token;
...
//     // Obtener y mostrar el historial de llamadas
//     function fetchCallHistory() {
//         log('Solicitando historial de llamadas...');
//         callHistoryLogDiv.innerHTML = ''; // Limpia el historial anterior

//         // backend para obtener el historial de llamadas de forma segura
//         $.getJSON('https://back-twilio-54644131242.us-central1.run.app/call-history')
//             .done(function (history) {
//                 if (history.length > 0) {
//                     log(`Historial de llamadas obtenido. Mostrando ${history.length} registros.`);
//                     let historyHTML = '<table><thead><tr><th>Destino</th><th>Duración</th><th>Fecha</th><th>Transcripción</th></tr></thead><tbody>';
//                     history.forEach(call => {
//                         const callDate = new Date(call.date).toLocaleString('es-CO');
//                         const callDuration = `${call.duration} seg`;
//                         const transcription = call.transcription || 'No disponible';
//                         historyHTML += `
//                             <tr>
//                                 <td>${call.to}</td>
//                                 <td>${callDuration}</td>
...
$(function () {
    ...
    const userToProcessInput = document.getElementById('user-to-process-input');
    ...
    function handleIncomingCall(call) {
        ...
        call.on('accept', handleAcceptedIncomingCall);
        call.on('cancel', handleDisconnectedIncomingCall);
        call.on('disconnect', handleDisconnectedIncomingCall);
        call.on('reject', handleDisconnectedIncomingCall);
    }
    ...
    async function makeOutgoingCall() {
        // ENVIAMOS LOS DATOS ADICIONALES EN LOS PARÁMETROS
        const params = {
            To: phoneNumberInput.value,
            user_id: userIdInput.value,
            doctor_id: doctorIdInput.value,
            user_to_process: userToProcessInput.value
        };
        ...
        if (device) {
            ...
            const call = await device.connect({
                url: 'https://back-twilio-54644131242.us-central1.run.app/voice',
                params: params // LOS PARÁMETROS SE AÑADEN AQUÍ
            });
            call.on('accept', updateUIAcceptedOutgoingCall);
            call.on('disconnect', updateUIDisconnectedOutgoingCall);
            call.on('cancel', updateUIDisconnectedOutgoingCall);
